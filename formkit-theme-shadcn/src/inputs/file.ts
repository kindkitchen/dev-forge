export default {
  ...
  suffixIcon: ``,
  inner: `
    relative ${"" /** position relative so that we can cover the whole inner with an invisible input */}
    ${""
  ...
  fileRemove: `
    ...
    items-center
    text-$colorTemperature-$colorTemperatureStrength
    text-[0px] ${"" /** on single inputs hide the remove text because we have an icon */}
    peer-data-[has-multiple]:text-xs ${"" /** on multiple inputs show the remove text */}
    peer-data-[has-multiple]:shrink-0 ${"" /** prevents wrapping for remove text */}
