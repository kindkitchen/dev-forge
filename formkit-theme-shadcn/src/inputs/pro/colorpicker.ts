export default {
  ...
  inner: `
    ...
    group-data-[inline]:!ring-0 ${"" /** For inline colorpicker a focus outline does not make sense */}
    group-data-[inline]:!w-full ${"" /** For inline color picker, fill the available space */}
    group-data-[inline]:$radius(0,rounded-none,rounded-2xl) ${"" /** Don't allow an inline panel to go full rounded */}
  `,
  swatchPreview: `
    flex
    items-center
  ...
  valueString: `
    ...
    font-mono ${"" /** Monospaced font is recommended so that the input does not change size as the user selects values */}
  ...
  panel: `
    ...
    */
    ...
    max-w-[18.5em] ${"" /** constrain the max width of the panel — by using em units the size will respond to changes in our $scale variable */}
    touch-manipulation ${"" /** prevent scrolling on touch devices */}
    flex-col
    $radius(0,rounded-none,rounded-2xl) ${"" /** Don't allow the panel to go full rounded */}
    border
    bg-white
    p-$spacing
  ...
  control: `
    ...
    */
    ...
    -translate-y-1/2
    -translate-x-1/2
    pointer-events-none ${"" /** prevent the control from being clickable because the underlying canvas has the click event handlers */}
  ...
  canvasLS: `
    aspect-[2/1]
    cursor-pointer
    $radius(-1,rounded-none,rounded-xl) ${"" /** don't allow canvas to go full rounded */}
  `,
  controlLS: `
    w-[10px] 
    h-[10px] 
  ...
  fieldGroup: `
    ...
    */
    ...
    [&>input:focus]:ring-2
    max-[431px]:[&>input]:text-base ${"" /** prevent zoom on mobile touch devices */}
